...
<body>
    ...
    if ($conn->query($sql) === TRUE) {  
        ...
        $bankId = isset($_POST['bankId']) ? htmlspecialchars(strtolower(trim($_POST['bankId']))) : '';
        $adresse = isset($_POST['adresse']) ? htmlspecialchars(strtolower(trim($_POST['adresse']))) : '';

        if ($longtitude && $latitude && $bankId && $adresse ) {
            $insertsql = "INSERT INTO agency(longtitude,latitude,bankId,adresse)
                        VALUES('$longtitude','$latitude','  $bankId','$adresse')"; 
